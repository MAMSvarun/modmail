...
module.exports.run = async(Client, message, args, prefix) => {
    ...
    try {
        ...
        if(userData.blocked == false) return message.reply({content: `**That user is not blocked from using modmail.**`}).then(msg => {
            setTimeout(() => {
                msg.delete()
                message.delete()
            }, 5000);   
        ...
        message.channel.send({embeds: [
            ...
            .setColor(`GREEN`)
            .setDescription(`**Unblocked** ***${member.user.tag}*** **from using modmail.**`)
            .setTimestamp()
        ]})
